...
function initPracticeSection() {
  ...
  filterButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (window.navManager) window.navManager.updateState({ filter: currentFilter, tab: 'practice' });
      currentPage = 1;
      renderProblems();
    });
  });
...
let currentPage = 1;
let currentFilter = window.navManager ? window.navManager.state.filter : 'all';
...
// QUIZ EDITOR
// ============================================
function openQuizEditor(problem, skipPushState = false) {
  currentProblem = problem;
  if (!skipPushState && window.navManager) {
    window.navManager.updateState({ problem: problem.id });
    window.navManager.setDynamicLabel(problem.title);
  }
...
function closeQuizEditor(skipPushState = false) { const el = document.getElementById("quizEditorModal"); if (el) el.classList.remove("active"); currentProblem = null; if (!skipPushState && window.navManager) window.navManager.updateState({ problem: null }); }
